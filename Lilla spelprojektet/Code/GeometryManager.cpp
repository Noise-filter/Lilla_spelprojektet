...
void GeometryManager::init(ID3D11Device *device)
{
	...
	nrOfVerts = 0;
	MESH_PNUV* node3 = importer->getOBJfromFile("Meshar/Green node.obj", nrOfVerts);
	...
	nrOfVerts = 0;
	MESH_PNUV* proj = importer->getOBJfromFile("Meshar/Very basic disc (projectile).obj", nrOfVerts);
